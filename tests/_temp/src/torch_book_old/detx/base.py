...
class DetectorABC(ABC):
    ...
    Args:
        name: detector title
        cfg: DETECTOR config for detector setting
        input_tensor_size: size of the tensor to input the detector. Square (x*x).
    ...
    def load(self, model_weights: str, **args):
        ...
        Args:
            ...
            args:
                classes_path
                config_file: the config file of detector
        """
        self.detector = ...
    ...
    def gradient_opt(self):
        ...
        self.train()
        self.ori_model = deepcopy(self.detector)
        self.optimizer = torch.optim.SGD(
            self.detector.parameters(), lr=1e-5, momentum=0.9, nesterov=True)
        self.optimizer.zero_grad()

    def reset_model(self):
        assert self.cfg.PERTURB.GATE == 'grad_descend'
        self.detector = deepcopy(self.ori_model)
