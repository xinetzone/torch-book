...
def rf_aggregnd(module: _ConvNd | _MaxPoolNd | _AvgPoolNd, inp: Tensor, out: Tensor) -> tuple[float, float, float]:
    k = module.kernel_size[0] if isinstance(module.kernel_size, tuple) else module.kernel_size
...
def rf_adaptive_poolnd(
    module: _AdaptiveMaxPoolNd | _AdaptiveAvgPoolNd, inp: Tensor, out: Tensor
) -> tuple[int, int, float]:
